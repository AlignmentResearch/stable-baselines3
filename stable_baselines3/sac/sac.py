    ...
    def train(self, gradient_steps: int, batch_size: int = 64) -> None:
        ...
        for gradient_step in range(gradient_steps):
            ...
            actions_pi, log_prob = self.actor.action_log_prob(
                replay_data.observations, recurrent_state=recurrent_states
            ).discard_state(self._no_state_error)
            ...
            with th.no_grad():
                ...
                next_actions, next_log_prob = self.actor.action_log_prob(
                    replay_data.next_observations, recurrent_state=recurrent_states
                ).discard_state(self._no_state_error)
                # Compute the next Q values: min over all critics targets
                next_q_values = th.cat(
                    self.critic_target(
                        replay_data.next_observations, next_actions, recurrent_state=recurrent_states
                    ).discard_state(self._no_state_error),
                    dim=1,
                )
            ...
            current_q_values = self.critic(
                replay_data.observations, replay_data.actions, recurrent_state=recurrent_states
            ).discard_state(self._no_state_error)
            ...
            q_values_pi = th.cat(
                self.critic(replay_data.observations, actions_pi, recurrent_state=recurrent_states).discard_state(
                    self._no_state_error
                ),
                dim=1,
            )
