...
def test_get_distribution(dummy_model_distribution_obs_and_actions):
    ...
    recurrent_states = model.initial_state()
    with th.no_grad():
        observations, _ = model.policy.obs_to_tensor(random_obs)
        actions = th.tensor(random_actions, device=observations.device).float()
        _, log_prob_1, entropy_1 = model.policy.evaluate_actions(observations, actions, recurrent_states).out
        distribution = model.policy.get_distribution(observations, recurrent_states).out
...
def test_predict_values(dummy_model_distribution_obs_and_actions):
    ...
    with th.no_grad():
        observations, _ = model.policy.obs_to_tensor(random_obs)
        actions = th.tensor(random_actions, device=observations.device).float()
        values_1, _, _ = model.policy.evaluate_actions(observations, actions, recurrent_states).out
        values_2 = model.policy.predict_values(observations, recurrent_states).out
