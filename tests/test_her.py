...
def test_truncate_last_trajectory(n_envs, recwarn, n_steps, handle_timeout_termination):
    ...
    replay_buffer = HerReplayBuffer(
        ...
        n_sampled_goal=2,
        goal_selection_strategy="future",
        extractor_state_example={"a": {"m": th.rand((3,))}},
    )
    ...
    for _ in range(n_steps):
        ...
        replay_buffer.add(observations, next_observations, actions, rewards, dones, infos, extractor_states={"a": {"m": th.rand((n_envs, 3,))}})
    ...
    assert np.allclose(old_replay_buffer.rewards[:pos], replay_buffer.rewards[:pos])
    assert np.allclose(old_replay_buffer.dones[: pos - 1], replay_buffer.dones[: pos - 1])
    assert np.allclose(old_replay_buffer.extractor_states["a"]["m"][:pos], replay_buffer.extractor_states["a"]["m"][:pos])
    ...
    assert not np.allclose(old_replay_buffer.extractor_states["a"]["m"][pos:end_pos], replay_buffer.extractor_states["a"]["m"][pos:end_pos])
    ...
    assert np.allclose(old_replay_buffer.extractor_states["a"]["m"][end_pos:], replay_buffer.extractor_states["a"]["m"][end_pos:])
