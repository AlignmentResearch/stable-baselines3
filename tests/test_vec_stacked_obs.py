...
def test_reset_update_box():
    ...
    observations_1 = obs_as_tensor(np.stack([space.sample() for _ in range(NUM_ENVS)]))
    ...
    observations_2 = obs_as_tensor(np.stack([space.sample() for _ in range(NUM_ENVS)]))
...
def test_episode_termination_box():
    ...
    dones[1] = True
    observations_3 = obs_as_tensor(np.stack([space.sample() for _ in range(NUM_ENVS)]))
